...
# Enhanced cross-encoder training script with YAML configuration management
# Usage: ./train_cross.sh [model_name]
# Example: ./train_cross.sh "castorini/monot5-base-msmarco"
...
print_usage() {
    ...
    echo ""
    echo "Available models in configs/cross_encoder.yaml:"
    python3 -c "import yaml; config = yaml.safe_load(open('configs/cross_encoder.yaml')); [print(f'  - {model}') for model in config.keys()]"
}

# Function to load YAML configuration
load_yaml_config() {
    local model_name="$1"
    local config_file="configs/cross_encoder.yaml"
...
# Load configuration
echo "=== Cross-Encoder Training Script Started ==="
