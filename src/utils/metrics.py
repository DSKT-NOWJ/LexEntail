...
class Metrics:
    ...
    def compute_all_metrics(
        self, all_ground_truths: list[list[int]], all_results: list[list[int]]
    ) -> dict:
        """
        ...
        for k in self.recall_at_k:
            scores[f"recall@{k}"] = self.compute_mean_score(
                self.recall, all_ground_truths, all_results, k
            )
        for k in self.map_at_k:
            scores[f"map@{k}"] = self.compute_mean_score(
                self.average_precision, all_ground_truths, all_results, k
            )
        for k in self.mrr_at_k:
            scores[f"mrr@{k}"] = self.compute_mean_score(
                self.reciprocal_rank, all_ground_truths, all_results, k
            )
        for k in self.ndcg_at_k:
            scores[f"ndcg@{k}"] = self.compute_mean_score(
                self.ndcg, all_ground_truths, all_results, k
            )
        scores["r-precision"] = self.compute_mean_score(
            self.r_precision, all_ground_truths, all_results
        )
        return scores
    ...
    def ndcg(self, ground_truths: list[int], results: list[int], k: int = None):
        ...
        dcg = relevances[0] + sum(
            relevances[i] / np.log2(i + 1) for i in range(1, len(relevances))
        )
